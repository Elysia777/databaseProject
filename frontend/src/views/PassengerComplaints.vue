<template>
  <div class="passenger-complaints">
      ...
      <div v-else class="complaints-list">
        <el-card v-for="complaint in complaints" :key="complaint.id" class="complaint-card" @click="showComplaintDetail(complaint)">
          ...
          <div class="complaint-content">
            <!-- 关联订单信息 -->
            <div v-if="complaint.orderNumber" class="order-summary">
              <div class="order-info-row">
                <el-icon><Document /></el-icon>
                <span class="order-number">订单号: {{ complaint.orderNumber }}</span>
                <el-tag v-if="complaint.paymentStatus" :type="getPaymentStatusType(complaint.paymentStatus)" size="small">
                  {{ getPaymentStatusText(complaint.paymentStatus) }}
                </el-tag>
              </div>
              <div class="route-info">
                <div class="route-item">
                  <el-icon class="pickup-icon"><Location /></el-icon>
                  <span class="route-text">{{ complaint.pickupAddress || '起点未知' }}</span>
                </div>
                <div class="route-arrow">→</div>
                <div class="route-item">
                  <el-icon class="destination-icon"><Position /></el-icon>
                  <span class="route-text">{{ complaint.destinationAddress || '终点未知' }}</span>
                </div>
              </div>
              <div v-if="complaint.actualFare" class="fare-info">
                <span class="fare-label">订单金额:</span>
                <span class="fare-amount">¥{{ complaint.actualFare.toFixed(2) }}</span>
                <span v-if="complaint.refundAmount && complaint.refundAmount > 0" class="refund-info">
                  (已退款 ¥{{ complaint.refundAmount.toFixed(2) }})
                </span>
              </div>
            </div>
    ...
    <!-- 创建投诉对话框 -->
    <el-dialog v-model="showCreateDialog" title="提交投诉" width="600px">
        ...
        <el-form-item label="关联订单" prop="orderId">
          <el-select v-model="complaintForm.orderId" placeholder="选择要投诉的订单" style="width: 100%;" @change="handleOrderChange">
            <el-option 
              ...
              :key="order.id" 
              :label="`${order.orderNumber} - ${order.destinationAddress}`" 
              :value="order.id">
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog v-model="showDetailDialog" title="投诉详情" width="700px">
      <div v-if="selectedComplaint" class="complaint-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="退款金额" v-if="selectedComplaint.refundAmount && selectedComplaint.refundAmount > 0">
            <span class="refund-amount">¥{{ selectedComplaint.refundAmount.toFixed(2) }}</span>
          </el-descriptions-item>
        </el-descriptions>

        <!-- 关联订单信息 -->
        <div v-if="selectedComplaint.orderInfo" class="order-info">
          <h4>关联订单信息</h4>
          <el-descriptions :column="2" border>
            <el-descriptions-item label="订单号">{{ selectedComplaint.orderInfo.orderNumber }}</el-descriptions-item>
            <el-descriptions-item label="订单金额">¥{{ selectedComplaint.orderInfo.actualFare || '0.00' }}</el-descriptions-item>
            <el-descriptions-item label="支付状态">
              <el-tag :type="getPaymentStatusType(selectedComplaint.orderInfo.paymentStatus)">
                {{ getPaymentStatusText(selectedComplaint.orderInfo.paymentStatus) }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="订单状态">
              <el-tag :type="getOrderStatusType(selectedComplaint.orderInfo.status)">
                {{ getOrderStatusText(selectedComplaint.orderInfo.status) }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="起点地址" :span="2">{{ selectedComplaint.orderInfo.pickupAddress || '无' }}</el-descriptions-item>
            <el-descriptions-item label="终点地址" :span="2">{{ selectedComplaint.orderInfo.destinationAddress || '无' }}</el-descriptions-item>
            <el-descriptions-item label="完成时间" v-if="selectedComplaint.orderInfo.completionTime">
              {{ formatDateTime(selectedComplaint.orderInfo.completionTime) }}
            </el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
...
const handleOrderChange = (orderId) => {
  ...
  if (selectedOrder && selectedOrder.driverId) {
    complaintForm.value.defendantId = selectedOrder.driverId
  }
}
...
// 显示投诉详情
const showComplaintDetail = async (complaint) => {
  selectedComplaint.value = complaint
  
  // 如果投诉有关联订单，获取订单详细信息
  if (complaint.orderId) {
    try {
      const response = await fetch(`/api/orders/${complaint.orderId}`, {
        headers: {
          'Authorization': `Bearer ${userStore.token}`
        }
      })
      ...
      const result = await response.json()
      if (result.code === 200) {
        selectedComplaint.value.orderInfo = result.data
        console.log('获取到关联订单信息:', result.data)
      } else {
